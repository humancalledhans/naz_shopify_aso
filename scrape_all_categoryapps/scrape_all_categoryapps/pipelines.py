...
class ScrapeAllCategoryappsPipeline:
    ...
    def upload_to_db(self, categoryapp_data):
        ...
        columns = 'AaT3C~*~GA@PQT'.join(str(x).replace('/', '_') for x in categoryapp_data.keys())
        values = 'AaT7C~*~GA@PQT'.join(str(x).replace('/', '_') for x in categoryapp_data.values())
        ...
        insert_stmt = """
            REPLACE INTO category_app ( category_id, app_id, date_scraped ) VALUES ( %s, %s, %s )
            """
