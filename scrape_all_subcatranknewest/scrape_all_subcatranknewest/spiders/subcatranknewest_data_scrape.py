        ...
        if not is_category:
            if "&sort_by=newest" in driver.current_url:
                for app_id in app_id_ranking_list:
                    return SubCategoryRankingNewest(subcategory_id=id, rank=app_id[0], app_id=app_id[1])


class SubcatranknewestDataScrapeSpider(scrapy.spiders.SitemapSpider):
    ...
    name = 'subcatranknewest_data_scrape'
    ...
    def close(spider, reason):
        ...
        categories_df = pd.read_csv(
            '/Users/hans/Desktop/Files/Non-Monash/Business/Working/2022/Main/Naz - Dev Apps/scraper_csv_files/AWS-Tester/subcategory_ranking_relevance.csv')
        categories_df.drop_duplicates(subset=['subcategory_id', 'app_id_list']).to_csv(
            '/Users/hans/Desktop/Files/Non-Monash/Business/Working/2022/Main/Naz - Dev Apps/scraper_csv_files/subcategory_ranking_relevance.csv', index=False)
