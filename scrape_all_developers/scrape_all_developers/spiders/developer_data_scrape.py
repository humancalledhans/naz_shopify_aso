...
class DeveloperDataScrapeSpider(scrapy.spiders.SitemapSpider):
    ...
    def parse_app_page(self, response):
        ...
        support_section_list = response.xpath("(//ul[@class='vc-app-listing-support-section__list'])[1]//text()[not(parent::style)]").getall()
        about_developer_section_list = response.xpath("(//ul[@class='vc-app-listing-support-section__list'])[3]//text()[not(parent::style)]").getall()
        ...
        for elem in support_section_list:
            ...
            if get_support_boolean:
                ...
                support_phone_number_regex = re.compile(r"^[+]*[-\s\./0-9]*[(]{0,1}[0-9]{1,4}[)]{0,1}[-\s\./0-9]*$")
                support_phone_processed = support_phone_number_regex.search(elem)
        ...
        dev_partners_href = response.xpath("((//ul[@class='vc-app-listing-support-section__list'])[3]//li[@class='vc-app-listing-support-section__list-item'])[1]//@href").get()
        ...
        if website_exists_boolean:
            all_hrefs_in_about_dev_tab = response.xpath(f"((//ul[@class='vc-app-listing-support-section__list'])[3]//li[@class='vc-app-listing-support-section__list-item'])//@href").getall()

            dev_website = all_hrefs_in_about_dev_tab[-1]

        if dev_partners_href is not None:
            dev_id = hashlib.md5(dev_partners_href.lower().encode()).hexdigest()
    ...
    def parse_apps_associated_with_developer(self, response):
        ...
        developed_apps_href = response.xpath("//div[@class='grid__item grid__item--tablet-up-half grid__item--desktop-up-quarter grid-item--app-card-listing']//a//@href").getall()
        ...
        yield Developer(
            ...
            developed_apps = developed_apps_list
            )
