...
def scrape_category_rankings(browse_page_url, id, is_category, end_link_to_append, sort_by_to_append):
    ...
    for page_num in range(1, int(number_of_apps)//24+2):
        ...
        # apps_href_list_length gets the number of listings on the current page.
        ...
        if is_category:
            if "&sort_by=relevance" in driver.current_url:
                for app_id in app_id_ranking_list:
                    yield CategoryRankingRelevance(category_id=id, ranking=app_id[0], app_id=app_id[1])


class CatrankrelevanceDataScrapeSpider(scrapy.spiders.SitemapSpider):

    CATEGORIES_REGEX = r"https://apps.shopify.com/categories/((\w+)(-\w+)*)[^?]"

    BASE_DOMAIN = "apps.shopify.com"

    name = 'scrape_all_catrankrelevance_dup'
