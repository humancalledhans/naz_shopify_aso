    ...
    if len(app_id_ranking_list) > 0:
        if not is_category:
            if "&sort_by=installed" in driver.current_url:
                return SubCategoryRankingInstalled(subcategory_id=id, app_id_list=app_id_ranking_list)
...
class SubcatrankinstalledDataScrapeSpider(scrapy.spiders.SitemapSpider):
    ...
    custom_settings = {
        ...
        'DOWNLOAD_DELAY': 3, # this should be the default parameter, when actually running the code.
    }
