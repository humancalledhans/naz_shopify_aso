    ...
    if len(app_id_ranking_list) > 0:
            if "&sort_by=installed" in driver.current_url:
                for app_id in app_id_ranking_list:
                    return CollectionRankingInstalled(collection_id=id, ranking=app_id[0], app_id=app_id[1])


class ColrankinstalledDataScrapeSpider(scrapy.spiders.SitemapSpider):

    COLLECTIONS_REGEX = r"https://apps.shopify.com/collections/((\w+)(-\w+)*)[^?]"

    BASE_DOMAIN = "apps.shopify.com"

    name = 'colrankinstalled_data_scrape'
    ...
    def close(spider, reason):
        ...
        collections_df = pd.read_csv('/Users/hans/Desktop/Files/Non-Monash/Business/Working/2022/Main/Naz - Dev Apps/scraper_csv_files/AWS-Tester/collections_ranking_installed.csv')
        collections_df.drop_duplicates(subset=['collection_id', 'app_ranking_list']).to_csv('/Users/hans/Desktop/Files/Non-Monash/Business/Working/2022/Main/Naz - Dev Apps/scraper_csv_files/collections_ranking_installed.csv', index=False)
