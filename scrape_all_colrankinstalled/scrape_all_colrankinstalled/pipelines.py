...
class ScrapeAllColrankinstalledPipeline:
    ...
    def upload_to_db(self, col_rank_installed_data):
        ...
        columns = 'AaT3C~*~GA@PQT'.join(str(x).replace('/', '_') for x in col_rank_installed_data.keys())
        values = 'AaT7C~*~GA@PQT'.join(str(x).replace('/', '_') for x in col_rank_installed_data.values())
        ...
        insert_stmt = """
            INSERT INTO col_rank_installed_data ( collection_id, ranking, app_id, date_time_scraped ) VALUES ( %s, %s, %s, %s )
            """
